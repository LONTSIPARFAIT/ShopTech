    ...
    <div class="space-y-6">
        ...
        <div class="rounded-xl border border-border bg-card p-6 shadow-sm">
            <div class="mb-6 flex items-center gap-3 border-b border-border pb-4">
                ...
                <div>
                    <h2 class="font-semibold text-foreground">Modifier le mot de passe</h2>
                    <p class="text-xs text-muted-foreground">Choisissez un mot de passe long et sécurisé</p>
                </div>
            </div>
            ...
            <Form v-bind="SecurityController.update.form()" :options="{ preserveScroll: true }" reset-on-success
                ...
                <div class="space-y-2">
                    ...
                    <PasswordInput id="current_password" name="current_password"
                        class="h-11 w-full rounded-lg bg-secondary border-border focus:border-primary focus:ring-2 focus:ring-primary/20"
                ...
                <div class="space-y-2">
                    <Label for="password_confirmation" class="text-sm font-medium text-foreground">Confirmer le mot de passe</Label>
                ...
                <Button :disabled="processing"
                    class="mt-4 w-full rounded-lg bg-gradient-to-r from-primary to-primary-dark py-2 font-semibold text-white transition-all hover:-translate-y-0.5 hover:shadow-lg">
        ...
        <div v-if="canManageTwoFactor" class="rounded-xl border border-border bg-card p-6 shadow-sm">
            <div class="mb-6 flex items-center gap-3 border-b border-border pb-4">
                <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary">
                    <Key class="h-5 w-5" />
                </div>
                <div>
                    <h2 class="font-semibold text-foreground">Authentification à deux facteurs</h2>
                    <p class="text-xs text-muted-foreground">Sécurisez votre compte avec une double vérification</p>
                </div>
            </div>
            ...
            <div v-if="!twoFactorEnabled" class="space-y-4">
                ...
                <Button v-if="hasSetupData" @click="showSetupModal = true"
                    class="rounded-lg bg-gradient-to-r from-primary to-primary-dark px-4 py-2 font-medium text-white transition-all hover:-translate-y-0.5 hover:shadow-lg">
